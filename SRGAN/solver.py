...
class SRGANTrainer(object):
    ...
    def save(self):
        g_model_out_path = "saved_weights/SRGAN_model_path.pth"
        d_model_out_path = "saved_weights/SRGAN_Discriminator_model_path.pth"
    ...
    def train(self):
        ...
        return g_train_loss / len(self.training_loader)
    ...
    def run(self):
        ...
        for epoch in range(1, self.nEpochs + 1):
            ...
            lst_loss.append(self.train())
            self.test()
            self.scheduler.step(epoch)
            if epoch == self.nEpochs:
                self.save()
        pt.plot(range(self.nEpochs), lst_loss)
        pt.ylabel('Loss')
        pt.xlabel('No of Epochs')
        pt.savefig('results/SRGAN.png')
        pt.close()